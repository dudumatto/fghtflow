...
import type { ApiPage, CompeticaoResponse } from "../services/types";
import { useAuth } from "../state/auth";

export default function CompeticoesPage() {
  const auth = useAuth();
  const [pageData, setPageData] = useState<ApiPage<CompeticaoResponse> | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const d = await api.get<ApiPage<CompeticaoResponse>>(`/competicoes?${qs.toString()}`);
        if (!alive) return;
        setPageData(d);
        setErr(null);
      } catch (e) {
  ...
  async function create() {
    ...
    setErr(null);
    try {
      const created = await api.post<CompeticaoResponse>("/competicoes", {
        nome,
        local: local || null,
  ...
  return (
    <div className="space-y-3">
      <div className="mb-1">
        <div className="text-xl font-semibold">Competições</div>
        <div className="mt-1 text-sm text-muted">Agenda e registros</div>
      </div>
        ...
        <Card>
          ...
          <div className="mt-3">
            <Button onClick={create} disabled={saving || !nome.trim()}>
              {saving ? <Spinner /> : "Criar competição"}
            </Button>
          </div>
        </Card>
      ) : null}
      ...
      <Card>
        {items.length === 0 ? (
          <div className="text-sm text-muted">Nenhuma competição cadastrada.</div>
        ) : (
